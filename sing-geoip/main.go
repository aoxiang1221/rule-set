...
func get(downloadURL *string, logflag bool) ([]byte, error) {
	if logflag {
...
func download(release *github.RepositoryRelease, logflag bool) ([]byte, error) {
	...
	return get(geoipAsset.BrowserDownloadURL, logflag)
}
...
func release(source string, output string, ruleSetOutput string, logflag bool) error {
	sourceRelease, err := fetch(source)
	if err != nil {
		return err
	}
	binary, err := download(sourceRelease, logflag)
	if err != nil {
		return err
	}
	...
	absRuleSet, err := filepath.Abs(ruleSetOutput)
	if err != nil {
		return err
	}
...
var (
	outputFile = flag.String("o", "geoip.db", "output geoip.db file")
...
func main() {
	flag.Parse()
	err := release("Loyalsoldier/geoip", *outputFile, *ruleSet, *logFlag)
	if err != nil {
		log.Fatal(err)
	}
}
